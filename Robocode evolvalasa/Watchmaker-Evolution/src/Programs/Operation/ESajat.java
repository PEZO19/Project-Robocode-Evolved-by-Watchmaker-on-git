 ...
 */
public class ESajat extends BinaryNode {

    double parameter;

    public ESajat(Node left, Node right) {
        super(left, right);
        Random random = new Random();
        double betweenNullAndOne = random.nextDouble();
        double scaled = betweenNullAndOne * 120;
        double shifted = scaled - 0;
        parameter = shifted;

        symbol = "ESa[" + parameter + "]";
    }

    @Override
    public void evaluate(BeliefBase beliefBase) {
        System.out.println("if elott");
        System.out.println("ESajat: " + BeliefBase.Esajat);
        if(BeliefBase.Esajat <= parameter){
